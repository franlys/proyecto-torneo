...
import { getDiscordBotStatusAction } from '@/lib/actions/discord-channels'
...
export function SyncDiscordButton({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function loadStatus() {
      try {
        const res = await getDiscordBotStatusAction(id)
        if (res && !('error' in res)) {
          setStatus(res)
        }
      } catch (err) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (result && 'error' in result) {
        ...
        const fresh = await getDiscordBotStatusAction(id)
        if (fresh) setStatus(fresh)
      } else if (result && 'message' in result) {
        toast.success(result.message)
        setStatus((prev) => (prev ? { ...prev, inGuild: true } : null))
        router.refresh()
      }
    } catch (err: any) {
  ...
  // If we detect the bot is NOT yet in the server, provide the direct 1-click invite link
  if (status && status.hasGuild && !status.inGuild) {
    return (
      <div className="flex items-center gap-2">
        <a
          ...
          title="Haz clic para autorizar e invitar al bot de Kronix a tu servidor en 1 clic"
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold
            text-white bg-gradient-to-r from-[#5865F2] to-neon-purple hover:brightness-110
            active:scale-[0.98] transition-all duration-150 shadow-lg shadow-[#5865F2]/20 font-orbitron animate-pulse"
        >
          ...
          <span>1. Invitar Bot a Servidor</span>
        </a>

        <button
          onClick={handleSync}
          disabled={isPending}
          title="Verificar y sincronizar después de invitar al bot"
          className="px-3 py-2.5 rounded-xl text-xs font-semibold text-white/70 bg-white/5 border border-white/10 hover:bg-white/10 active:scale-[0.98] transition-all font-orbitron"
        >
          {isPending ? 'Verificando...' : '2. Sincronizar'}
        </button>
      </div>
    )
  }
