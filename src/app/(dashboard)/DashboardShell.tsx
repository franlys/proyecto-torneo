...
export default function DashboardShell({
  ...
  // Collapsible sidebar folders states
  const [streamerOpen, setStreamerOpen] = useState(true)
  const [adminCentralOpen, setAdminCentralOpen] = useState(true)
  const [adminFinanceOpen, setAdminFinanceOpen] = useState(true)
    ...
    return (
      <div className="space-y-1">
        <button
          type="button"
          onClick={onToggle}
          className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-sm text-white/60 hover:text-white hover:bg-white/[0.02] transition-all duration-200 group text-left"
        >
          <div className="flex items-center gap-3">
            {icon}
            <span>{label}</span>
          </div>
          <ChevronRight className={`w-3.5 h-3.5 transition-transform duration-200 text-white/30 group-hover:text-white/60 shrink-0 ${isOpen ? 'rotate-90' : ''}`} />
        </button>
        ...
        <AnimatePresence initial={false}>
          {isOpen && (
            <motion.div
              ...
              className="pl-3 space-y-0.5 mt-0.5 border-l border-white/15 ml-5 overflow-hidden"
            >
              {children}
            </motion.div>
          )}
  ...
  const NavLinks = () => (
    <nav className="flex-1 px-3 py-4 space-y-1">
      ...
      {(userRole !== 'USER' || isStaff) && (
        <SidebarFolder
          label={userRole === 'USER' ? 'Mi Portal (Colaborador)' : 'Mi Portal (Organizador)'}
          icon={
            <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          isOpen={streamerOpen}
          onToggle={() => setStreamerOpen(!streamerOpen)}
        >
          <SidebarLink href="/kronix" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
            <span>🏆</span> {userRole === 'USER' ? 'Mis Inscripciones' : 'Mis Torneos'}
          </SidebarLink>
          
          {(userRole === 'STREAMER' || userRole === 'SUPER_ADMIN' || userRole === 'ADMIN') && (
            <>
              <SidebarLink href="/kronix/approvals" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                <span>✅</span> Aprobaciones
              </SidebarLink>
              <SidebarLink href="/kronix/staff" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                <span>👥</span> Mi Staff
              </SidebarLink>
              <SidebarLink href="/kronix/payments" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
          ...
          {userRole === 'STREAMER' && (
            <SidebarLink href="/kronix/support" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
              <span>🎧</span> Soporte Kronix
            </SidebarLink>
          )}
        </SidebarFolder>
      )}
          ...
          {(userRole === 'SUPER_ADMIN' || userRole === 'ADMIN') && (
            <>
              ...
              <SidebarFolder
                ...
                isOpen={adminCentralOpen}
                onToggle={() => setAdminCentralOpen(!adminCentralOpen)}
              >
                <SidebarLink href="/admin/kronix" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                  <span>🏆</span> Torneos Admin
                </SidebarLink>
                ...
                <SidebarLink href="/admin/settings" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                  <span>⚙️</span> Personalizar Inicio
                </SidebarLink>
              </SidebarFolder>

              {/* Finanzas y Métricas Folder */}
              <SidebarFolder
                label="Finanzas y Métricas"
                icon={
                  <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                  </svg>
                }
                isOpen={adminFinanceOpen}
                onToggle={() => setAdminFinanceOpen(!adminFinanceOpen)}
              >
                <SidebarLink href="/admin/finance" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                  <span>💰</span> Finanzas & Ingresos
                </SidebarLink>
                <SidebarLink href="/admin/analytics" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                  <span>📊</span> Analíticas
                </SidebarLink>
                <SidebarLink href="/admin/ads" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                  <span>📢</span> Publicidad
                </SidebarLink>
              </SidebarFolder>

              {/* Módulos Especiales Folder */}
              <SidebarFolder
                ...
                isOpen={adminModulesOpen}
                onToggle={() => setAdminModulesOpen(!adminModulesOpen)}
              >
                <SidebarLink href="/admin/raffles" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                  <span>🎟️</span> Gestión de Sorteos
                </SidebarLink>
                <SidebarLink href="/admin/bets" className="!py-1.5 !px-2.5 text-xs text-white/50 hover:text-white">
                  <span>🎰</span> Control Apuestas
                </SidebarLink>
              </SidebarFolder>
            </>
          )}
