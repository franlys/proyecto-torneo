...
export default async function AdminRevenuePage() {
  ...
  try {
    ...
    const [
      { data: approvedSubs, error: subsError },
      { count: activeTournaments, error: activeError },
                    ...
                    <tbody className="divide-y divide-white/5">
                      {acRecords.map((r: any) => (
                        <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-emerald-400 tabular-nums font-bold">
                            ${Number(r.rake_earned ?? 0).toFixed(2)}
                          </td>
    ...
    return <AdminErrorCard section="Control de Ingresos" error={err?.message || String(err)} />
  }
}
