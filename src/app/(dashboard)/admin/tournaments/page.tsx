...
export default async function AdminTournamentsPage() {
  try {
    ...
    // Fetch profiles to join in-memory and bypass the PostgREST relationship cache check
    ...
    const tournaments = (tournamentsData ?? []).map(t => ({
      ...t,
      profiles: {
        username: profileMap.get(t.creator_id) || null
      }
    }))
              ...
              {tournaments.map((t) => (
                <tr key={t.id} className="group hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4 text-[10px] font-black uppercase tracking-widest">
                    <span className={`px-2 py-1 rounded border ${statusColors[t.status as keyof typeof statusColors] || 'bg-white/5 text-white/30'}`}>
                      {t.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
    ...
    return <AdminErrorCard section="Panel de Torneos" error={err?.message || String(err)} />
  }
}
