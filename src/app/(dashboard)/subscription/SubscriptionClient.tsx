...
import { buySubscriptionWithCoins } from '@/lib/actions/subscription-coins'
...
export default function SubscriptionClient({ initialStatus, initialExpiry, role, initialBalance }: SubscriptionClientProps) {
  const router = useRouter()
  const [status, setStatus] = useState(initialStatus)
  const [expiry, setExpiry] = useState(initialExpiry)
  const [balance, setBalance] = useState(initialBalance)
  const [exchangeRate, setExchangeRate] = useState(58.25)
  const [activeTab, setActiveTab] = useState<'paypal' | 'kcoins'>('paypal')
  ...
  useEffect(() => {
    fetch('https://open.er-api.com/v6/latest/USD')
      ...
      .catch(err => console.error('Error fetching rate on client:', err))
  }, [])
  ...
  useEffect(() => {
    if (!document.getElementById('paypal-sdk')) {
      ...
      script.src = `https://www.paypal.com/sdk/js?client-id=${process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID}&currency=USD&enable-funding=card,paylater,venmo`
      script.async = true
      document.body.appendChild(script)
    }
  }, [])

  const handlePayWithCoins = async (planId: string) => {
    const plan = PLANS.find(p => p.id === planId)
    if (!plan) return

    const costCoins = parseFloat((plan.amount * exchangeRate).toFixed(2))
    if (balance < costCoins) {
      toast.error(`Saldo insuficiente. El plan cuesta 🪙 ${costCoins.toLocaleString('es-DO')} K-Coins y tu saldo es de 🪙 ${balance.toLocaleString('es-DO')} K-Coins.`)
      return
    }

    if (!confirm(`¿Confirmas la compra del pase VIP "${plan.title}" por 🪙 ${costCoins.toLocaleString('es-DO')} K-Coins?`)) {
      return
    }

    setIsPayingWithCoins(true)
    try {
      const res = await buySubscriptionWithCoins(planId)
      if ('error' in res) {
        toast.error(res.error)
      } else {
        setStatus('ACTIVE')
        if (res.expiry) setExpiry(res.expiry)
        setBalance(prev => parseFloat((prev - costCoins).toFixed(2)))
        setShowPaypalModal(false)
        setShowSuccessModal(true)
        router.refresh()
      }
            ...
            >
              ...
              <div className="bg-[#0d0f15] h-full rounded-3xl p-6 flex flex-col cursor-pointer">
                <div className="text-center mb-6">
                  ...
                  <div className="mt-1.5 text-[#009cde] font-orbitron font-bold text-xs uppercase tracking-wider">
                    <span>🪙 {Math.round(plan.amount * exchangeRate)} K-Coins</span>
                  </div>
                </div>
          ...
          <div className="relative w-full max-w-sm sm:max-w-2xl bg-gradient-to-br from-[#0d0f15] to-[#080a0e] rounded-3xl border border-white/10 shadow-[0_0_80px_rgba(0,156,222,0.15)] overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <div className="flex-1 flex flex-col sm:flex-row overflow-hidden">
              ...
              <div className="flex-1 p-6 space-y-5 flex flex-col justify-start bg-white relative overflow-y-auto">
                ...
                {activeTab === 'paypal' ? (
                  <>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest text-center font-bold">Pagar con tu saldo de PayPal o Tarjeta</p>
                  ...
                  <div className="space-y-4 py-2">
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest text-center font-bold">Pagar con el saldo de tu Billetera</p>
                    
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 space-y-3">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-gray-500">Tu Saldo Disponible:</span>
                        <span className="font-orbitron font-bold text-gray-800">🪙 {balance.toLocaleString('es-DO')} K-Coins</span>
                      </div>
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-gray-500">Costo del Pase:</span>
                        <span className="font-orbitron font-bold text-[#009cde]">🪙 {Math.round(activePlanDetails.amount * exchangeRate).toLocaleString('es-DO')} K-Coins</span>
                      </div>
                      <div className="border-t border-gray-200 pt-2 flex justify-between items-center text-xs font-bold">
                        ...
                        <span className={`font-orbitron ${
                          balance >= Math.round(activePlanDetails.amount * exchangeRate)
                            ? 'text-green-600'
                            : 'text-red-500'
                        }`}>
                          🪙 {(balance - Math.round(activePlanDetails.amount * exchangeRate)).toLocaleString('es-DO')} K-Coins
                        </span>
                      </div>
                    </div>
                    ...
                    {balance >= Math.round(activePlanDetails.amount * exchangeRate) ? (
                      <button
                        onClick={() => handlePayWithCoins(activePlanDetails.id)}
                        disabled={isPayingWithCoins}
                        className="w-full py-3.5 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-600 hover:to-amber-600 text-white font-bold rounded-xl uppercase tracking-widest text-xs shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                      >
                        {isPayingWithCoins ? (
                          <>
                      ...
                      <div className="space-y-3">
                        <div className="p-3 bg-red-50 border border-red-100 rounded-xl text-center">
                          <p className="text-[10px] text-red-600 font-bold leading-relaxed">
                            No tienes suficientes K-Coins para adquirir este pase VIP.
                          </p>
                        </div>
                        <Link
                          href="/wallet"
                          onClick={() => setShowPaypalModal(false)}
                          className="w-full py-3.5 bg-gray-900 hover:bg-black text-white font-bold rounded-xl uppercase tracking-widest text-xs transition-all text-center block"
                        >
                          Recargar K-Coins
                        </Link>
                      </div>
                ...
                <div className="flex items-center justify-center gap-2 pt-2">
                  ...
                  <span className="text-[10px] text-gray-400 font-medium">Procesado de forma segura</span>
                </div>
              </div>
            </div>
          </div>
