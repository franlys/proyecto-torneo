          ...
          {raffle.status === 'active' && !purchaseSuccess && (
            <form onSubmit={handlePurchase} className="space-y-6">
              ...
              {showPaypalModal && (
                ...
                >
                  {/* Modal: mobile = single column | desktop = two columns */}
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between items-start gap-2">
                              <span className="text-xs text-white/50 leading-snug">{ticketCount} boleto{ticketCount > 1 ? 's' : ''}</span>
                              <span className="text-xs font-bold text-white whitespace-nowrap">RD$ {totalCost.toFixed(2)}</span>
                            </div>
                            <p className="text-[10px] text-white/30 truncate">{raffle.title}</p>
                          </div>
                        </div>

                        <div className="border-t border-white/[0.06] pt-4">
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] text-white/30 uppercase tracking-widest">Total USD</span>
                            <span className="font-orbitron font-black text-[#009cde] text-xl">${(totalCost / usdToDopRate).toFixed(2)}</span>
                          </div>
                          <p className="text-[9px] text-white/20 mt-1">Tasa: 1 USD = {usdToDopRate.toFixed(2)} DOP</p>
                        </div>
